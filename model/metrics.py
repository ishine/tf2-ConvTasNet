...
def calc_sdr_tf(estimation, origin, mask=None):
    """
    batch-wise SDR caculation for one audio file on pytf Variables.
    estimation: (batch, nsample)
    origin: (batch, nsample)
    mask: optional, (batch, nsample), binary
    """
    ...
    origin_power = tf.pow(origin, 2).sum(1, keepdim=True) + 1e-8  # (batch, 1)

    scale = tf.sum(origin * estimation, 1, keepdim=True) / origin_power  # (batch, 1)

    est_true = scale * origin  # (batch, nsample)
    est_res = estimation - est_true  # (batch, nsample)

    true_power = tf.pow(est_true, 2).sum(1)
    res_power = tf.pow(est_res, 2).sum(1)

    return 10 * tf.log10(true_power) - 10 * tf.log10(res_power)  # (batch, 1)
...
def batch_SDR_tf(estimation, origin, mask=None):
    """
    batch-wise SDR caculation for multiple audio files.
    estimation: (batch, nsource, nsample)
    origin: (batch, nsource, nsample)
    ...
    # zero mean signals
    estimation = estimation - tf.mean(estimation, 2, keepdim=True).expand_as(estimation)
    origin = origin - tf.mean(origin, 2, keepdim=True).expand_as(estimation)

    # possible permutations
    perm = list(set(permutations(np.arange(nsource))))

    # pair-wise SDR
    SDR = tf.zeros((batch_size, nsource, nsource)).type(estimation.type())
    for i in range(nsource):
        for j in range(nsource):
            SDR[:, i, j] = calc_sdr_tf(estimation[:, i], origin[:, j], mask)
    ...
    for permute in perm:
        ...
        sdr = tf.sum(tf.cat(sdr, 1), 1)
        SDR_perm.append(sdr.view(batch_size, 1))
    SDR_perm = tf.cat(SDR_perm, 1)
    SDR_max, _ = tf.max(SDR_perm, dim=1)
...
def GetSDR(se, s):
    ...
    for jest in range(nsrc):
        for jtrue in range(nsrc):
            SDR[jest, jtrue], SIR[jest, jtrue], SAR[jest, jtrue] = compute_measures(
                se[:, jest], s, jtrue
            )
